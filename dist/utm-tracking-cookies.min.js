...
function getRootDomain(e){if(!e){return null}var d=e.split(".");var c;var a;if(d.length<2){return null}else{var b=1;while(b++){c=d.slice(-b);a=c.join(".");Cookies.set("$checkRootDomain","check",{domain:a});if("check"===Cookies.get("$checkRootDomain")){Cookies.remove("$checkRootDomain",{domain:a});break}}return a}}Cookies.defaults.expires=180;if(getRootDomain(document.domain)){Cookies.defaults.domain=getRootDomain(document.domain)}

function getParameterByName(a){a=a.replace(/[\[]/,"\\[").replace(/[\]]/,"\\]");var c=new RegExp("[\\?&]"+a+"=([^&#]*)"),b=c.exec(location.search);return b===null?"":decodeURIComponent(b[1].replace(/\+/g," "))};
